...
class PartialRewardEnv:
    ...
    def reset(self):
        ...
        s0 = self.best_arm / max(1, (self.num_arms - 1))  # 归一化
...
def test_nn_bandit_cumulative_regret(nn_bandit, num_test_episodes=100, steps_per_episode=30):
    """
    使用训练好的模型在新的测试环境下运行 num_test_episodes 个 episode，
    计算每个 episode 的累计遗憾（regret = optimal_reward (1) - obtained reward），
    并绘制累计遗憾图。
    ...
    And draw a cumulative regret map.
    """
    state_dim = 1
    num_arms_nn = 10
...
def compare_all_algos_in_partial_env():
    ...
    plt.xlabel("Time Steps")
    plt.ylabel("Cumulative Regret")
    plt.title("All Bandits on PartialRewardEnv (Cumulative Regret)")
    plt.legend()
    plt.show()
